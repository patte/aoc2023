...
fn main() {
    ...
    // roll out the map ranges into entries
    //for map in maps.iter_mut() {
    //    for range in &map.ranges {
    //        for i in 0..range.range_length {
    //            let entry = AlmanacMapEntry {
    //                source: range.source_range + i,
    //                destination: range.destination_range + i,
    //            };
    ...
    for seed in seeds {
        ...
        for map in maps.iter() {
            // 🪦 what you should not do:
            //let map_entry: Option<&AlmanacMapEntry> =
            //    map.entries.iter().find(|e| e.source == current_value);
            //if map_entry.is_some() {
            //    current_value = map_entry.unwrap().destination;
            //}
